...
- `eslint.ignoreOutsideWorkspace` - when enabled, local `file:` documents outside all open workspace folders are not validated. The default is `false`. Untitled, remote, and other virtual documents remain unchanged, and the setting has no effect when no workspace folder is open so that single-file mode keeps working.
