...
- `eslint.ignoreOutsideWorkspace` - when enabled, local files outside all open workspace folders are not validated. The default is `false`. The setting does not affect untitled or other virtual documents, and it has no effect when no workspace folder is open so that single-file mode keeps working.
