...
- `eslint.ignoreOutsideWorkspace` - when enabled, documents outside all open workspace folders are not validated. The default is `false`. Untitled documents remain controlled by `eslint.ignoreUntitled`, and the setting has no effect when no workspace folder is open so that single-file mode keeps working.
