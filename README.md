...
If using with multi-project workspace, the extension may not resolve config modules correctly. Thus you will need to manually resolve them in `.eslintrc.js`:

```javascript
// ESLint config file
// ref: https://eslint.org/docs/user-guide/configuring
module.exports = {
  extends: [require.resolve('shared-eslint-config')],
};
```
